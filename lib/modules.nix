...
let
  ...
  inherit (lib)
    filterAttrs hasSuffix mapAttrs' mkDefault mkOption nameValuePair nixosSystem
    removeSuffix types;
...
in rec {
  ...
  */
  mapModules' = dir: fn: dirfn:
    mapFilterAttrs (_: v: v != null) (name: type:
  ...
  mapModules = dir: fn:
    mapModules' dir fn
    (path: if pathExists "${path}/default.nix" then fn path else null);
